...
Every normal `main section.section-space` receives five decorative shapes at runtime. Layouts `1` through `5` are shuffled in cycles so adjacent sections do not repeat the same arrangement, and a new order is selected on each direct load or Astro navigation. Horizontal positions use the shared responsive safe inset so complete shapes remain visible after idle drift instead of being cropped at viewport edges. Standard generated shape fields paint above the section content so nearby surfaces cannot visually crop them, while their layouts keep them in transition whitespace and `pointer-events: none` keeps content interactive. A section may provide its own direct `data-section-shapes` layer when a semantic decorative field such as the stacked story's clouds should replace those generated shapes; that custom field may stay behind its content when the composition requires it. The shapes use separate parallax and idle wrappers so shared GSAP motion never competes for one transform. Repeated card-like components also receive one of five `data-organic-pattern-layout` arrangements, shuffled independently within each parent group; a complete set is used before a layout repeats, and adjacent cards never share the same arrangement. `SectionHeading`, shared card surfaces, `InfoCard`, rail cards, directory cards, callouts, the directory map, the membership form, and the footer also inherit or compose the same system. Reuse these treatments instead of adding page-local circles or blobs. Keep shapes decorative, pointer-transparent, positioned away from readable content, and clipped by the nearest safe paint boundary.

Direct child sections of `main` also receive a curved `data-section-transition` boundary at runtime. The boundary is attached to the outgoing section, filled with the incoming section's resolved surface color, and its SVG surface scrolls upward slightly to reveal that surface progressively. Foreground motifs remain on a separate stationary layer so paint containment cannot obscure them. Three Bézier profiles and alternating organic motifs keep consecutive joins varied. The page hero emits the same hook and shared Bézier markup for its `nextTone` boundary. Keep this transform layer separate from section-shape motion and preserve the reduced-motion fallback.
...
- `data-section-transition`, used by the hero curve and generated inter-section Bézier boundaries.
